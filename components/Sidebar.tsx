...
const Sidebar: React.FC<SidebarProps> = ({ activeLessonId, onLessonSelect, isOpen, onClose, userScores }) => {
  return (
    <div className={`
    ...
    `}>
      ...
      <div className="px-6 py-4">
        <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
          <div 
            ...
            style={{ width: `${(Object.keys(userScores).length / LESSONS.length) * 100}%` }}
          />
        </div>
        <div className="flex justify-between items-center mt-2">
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">الدروس المكتملة</p>
            <p className="text-[10px] text-emerald-600 font-bold">{Object.keys(userScores).length} من {LESSONS.length}</p>
        </div>
      </div>
      ...
      <div className="p-4 border-t bg-slate-50/50">
        <div className="p-3 bg-white rounded-xl border border-slate-200 shadow-sm text-center">
            <p className="text-[10px] text-slate-400 font-bold mb-1">المستوى الحالي</p>
            <div className="text-xs font-black text-slate-800">
                {Object.keys(userScores).length >= LESSONS.length ? '🎓 مبرمج محترف' : '🌱 طالب مبتدئ'}
            </div>
        </div>
      </div>
    </div>
